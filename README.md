...
- 从 2×2 木筏起步，逐步扩建至 3×3、4×4，并放置 11 种可见设施。
- 管理生命、饱食、口渴和船体耐久，收集 8 类资源。
- 利用收集网、过滤器、烤架和盐雾菜圃建立自动生产链。
- 在残骸带、渔场和避风航线之间选择不同的资源结构与风险回报。
- 经历昼夜、阴晴、风暴预警、船体损伤和随机海上事件。
...
- 海面由低频涌浪、交叉浪、碎浪与前景浪峰组成；木筏、漂浮物、浮标和游泳怪物共享同一水面采样。
...
钓鱼严格经过 `fishCast → fishWait → fishReel → idle`。同一指针从按下到释放只产生一个事务，快速双击、触控合成事件和键盘重复不会造成连续抛两次竿。
...
本地 `TideGameState` 是单人玩法的唯一运行时状态，存档保持 `schemaVersion: 2`。旧存档可迁移，游客身份和 `runId` 相互独立；重新开始会创建新 `runId`，但保留玩家身份和个人记录。
...
| 检查项 | 结果 |
| --- | --- |
| 游戏规则、水面、夜战、投影、八向移动、动作与输入测试 | 55/55 通过 |
